...
for( i in 1: Nrep) {
  res <- simula.funcionamento.primeiro.estagio.1(tol = 30)
  log <- res$log
  res <- res$ans
  ...
  # max das filas
  line.max <- c(line.max, log %>% summarise(max = max(line_1)) %>% .$max)
  line.max2 <- c(line.max2, log2 %>% summarise(max = max(line_1, line_2, line_3, line_4)) %>% .$max)

  # Respostas:
  n.natendidos <- c(n.natendidos, log$line_1[length(log$line_1)])
