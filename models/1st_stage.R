...
simula.funcionamento.primeiro.estagio.3 <- function(t.abre = 8, 
                                 t.fecha = 16,
                                 tol = 30
                                 ){
  log <- data.frame()
...
simula.funcionamento.primeiro.estagio.2 <- function(t.abre = 8, 
                                 t.fecha = 16,
                                 tol = 30
                                 ){
  log <- data.frame()
...
simula.funcionamento.primeiro.estagio.1 <- function(t.abre = 8, 
                                 t.fecha = 16,
                                 tol = 30
                                 ){
  log <- data.frame()
  ...
  get.in.line <- log %>% select(medico, t_chegada)
  get.in.line <- get.in.line %>% mutate(line_1 = ifelse(medico == 1, +1, 0)) %>% select(-medico)
  names(get.in.line) <- c("t", "line_1")

  get.out.line <- log %>% select(medico, t_inicio, atendido)
  get.out.line <- get.out.line %>% filter(atendido == TRUE) %>% mutate(line_1 = ifelse(medico == 1, -1, 0)) %>% select(-medico, -atendido)
  names(get.out.line) <- c("t", "line_1")
...
simula.funcionamento.config.c <- function(t.abre = 8, 
                                 t.fecha = 16,
                                 tol = 30
                                 ){
  log <- data.frame()
